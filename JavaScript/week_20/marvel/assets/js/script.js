...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    for(let hero of heroes) {
        heroCard += `<div class="heroes">
        <h2 class="title">${hero.name}</h2>
        <p class="text"><span class="bold">Вселенная: </span>${hero.universe}</p>
        <p class="text"><span class="bold">Альтер эго: </span>${hero.alter}</p>
        <p class="text">
        <span class="bold">Род деятельности: </span>${hero.occupation}
        </p>
        <p class="text"><span class="bold">Друзья: </span>${hero.friends}</p>
